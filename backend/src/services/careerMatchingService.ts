...
function getDimensionCategory(dimension: string): string {
...
type WeightedVector = Map<string, { value: number; weight: number }>;

function buildUserVector(scores: DimensionScore[]): WeightedVector {
...
function buildCareerVector(career: CareerProfile): WeightedVector {
...
function weightedEuclideanDistance(user: WeightedVector, career: WeightedVector): number {
  let sumSquared = 0;
  let totalWeight = 0;

  for (const [dim, { value: careerVal, weight }] of career) {
    const userEntry = user.get(dim);
    if (!userEntry) continue;

    const diff = userEntry.value - careerVal;
...
export function computeMatches(
  ...
  const matches = careers.map(career => {
    ...
    const matchPct = Math.max(0, (1.0 - distance / 100.0) * 100.0);
